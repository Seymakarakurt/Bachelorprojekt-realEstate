$(document).ready(function () {
  ...
  $(
    "#selectTags, #datepicker-from, #datepicker-to, #userSelect, #searchQuery"
  ).on("change", function () {
  ...
  function loadUser() {
    ...
    $.ajax({
      ...
      success: function (data) {
        // Was geschieht, wenn die Anfrage erfolgreich ist
      ...
      error: function () {
        // Was geschieht, wenn die Anfrage fehlschlägt
  ...
  function loadTags() {
    ...
    $.ajax({
      ...
      success: function (data) {
        // Funktion bei erfolgreichem Aufruf
  ...
  function initializeDropzone() {
    ...
    var myDropzone = new Dropzone("#dropzoneDocs", {
      ...
      acceptedFiles:
        ".png, .jpg, .jpeg, .webp, .pdf, .txt, .docx, .xlsx, .pptx", // Akzeptierte Dateitypen
  ...
  $("#tagForm").on("input", function () {
    ...
    $.ajax({
      ...
      success: function (data) {
        // Wenn der AJAX-Aufruf erfolgreich ist, führe diese Funktion aus
  ...
  $("#projektForm").on("input", function () {
    ...
    $.ajax({
      url: "/documents/autocomplete/projects/", // URL der Backend-Ressource
      data: { q: query }, // Die Eingabe des Benutzers wird als Parameter "q" übergeben.
      success: function (data) {
        // Funktion, die bei erfolgreicher Antwort aufgerufen wird.
