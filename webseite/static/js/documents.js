$(document).ready(function () {
  ...
  // Öffnet Modal-Fenster auf Aktuelles zum Dokument hochladen
    ...
    $.ajax({
      ...
      success: function (data) {
        ...
        data.documents.forEach((element) => {
          ...
          documentCheckWatchlistStatus(userLogin, documentId);
  ...
  // Dokumentenfilterung bei Änderung des Enddatums
  ...
  function documentCheckWatchlistStatus(user_id, documentId) {
    var user_id = user_id;
    var project_id = documentId;

    $.ajax({
      url: "/documents/check-watchlist/" + user_id + "/" + project_id + "/",
      type: "GET",
      dataType: "json",
      headers: {
        "X-CSRFToken": csrftoken,
  ...
  $("#documentCards").on("click", ".documentBookmark", function () {
    ...
    $.ajax({
      ...
      success: function (data) {
        documentCheckWatchlistStatus(user_id, document_id);
      },
    });
  });
